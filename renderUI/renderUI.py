...
class mainUI(QtWidgets.QDialog, main.Ui_Form):
    ...
    def confirmation(self, *args):
        reply = QtWidgets.QMessageBox.question(self, "Confirmation",
                                               "For this code to work your scene should be setup a certian way , where your objects have to be inside a group called 'geo' , your environments objects such as projection planes should be inside a group called 'env' and all your lights should be inside a group called 'lgt' . Have you organised yout Scene")
...
class beautyUI(QtWidgets.QDialog, stacked.Ui_Form):
        ...
        if self.count == 0:
            ...
            if self.shaderList.selectedItems():
                ...
                if self.nameShader.text() == "":
                    ...
                    reply = QtWidgets.QMessageBox.question(self, "Create Further Collection",
                                                           "Do you want to create further collections for this layer?")
                    if reply == QtWidgets.QMessageBox.Yes:
    ...
    def createLayerCollectionClose(self, *args):
        ...
        if self.check.isChecked():
            ...
            for a in range(len(aov_name)):
                aov = aov_name[a][6:]
                sub_colle = collection.create(self.r1.name() + '_' + aov, collection.AOVChildCollection.kTypeId,
                                              aovName=aov)
                aov_collection.appendChild(sub_colle)
                override = sub_colle.createAbsoluteOverride('aiAOV_' + aov, 'enabled')  # (aov name, attr name)
                override.setAttrValue(1)  # override value
        ...
        if initvalues[0]:
            ...
            for c in c1:
                if c.name() == initvalues[2]:
                    ...
                    target_collection = renderSetup.instance().getRenderLayer(initvalues[1]).getCollectionByName(
                        initvalues[2])
